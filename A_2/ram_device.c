...
#define DOR_DEVICE_SIZE 1024 /* sectors */
...
{
	dev_data = vmalloc(DOR_DEVICE_SIZE * DOR_SECTOR_SIZE);
	if (dev_data == NULL)
		return -ENOMEM;
	/* Setup its partition table */
	copy_mbr_n_br(dev_data);
	return DOR_DEVICE_SIZE;
}
...
{
	memcpy(dev_data + sector_off * DOR_SECTOR_SIZE, buffer,
		sectors * DOR_SECTOR_SIZE);
}
void ramdevice_read(sector_t sector_off, u8 *buffer, unsigned int sectors)
{
	memcpy(buffer, dev_data + sector_off * DOR_SECTOR_SIZE,
		sectors * DOR_SECTOR_SIZE);
}
